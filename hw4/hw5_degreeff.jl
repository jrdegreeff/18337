...
# ╔═╡ c03054ea-5223-427c-8a78-8b9f51a5b373
begin
...
# ╔═╡ 20a26f72-bf49-11ec-2002-33344e4e0070
md"""
# 6.338 HW 5 — Spring 2022
#### Jeremiah DeGreeff
"""

# ╔═╡ bb2cdfbc-e7d0-4535-832f-c0a7ded9f8e7
md"""
## Problem 1: Parameter Estimation in Dynamical Systems
"""

# ╔═╡ c2ca1a26-99a6-438a-9f17-f5b7042bbd17
md"""
### Part 1
"""

# ╔═╡ d128807f-7576-4fac-9cad-027d4215eb4f
md"""
$B_f^x(1) = [\overline{x_1}, \overline{x_2}, \dots] = [\overline{y}\frac{\partial y}{\partial x_1}, \overline{y}\frac{\partial y}{\partial x_2}, \dots] = [1\frac{\partial f(x)}{\partial x_1}, 1\frac{\partial f(x)}{\partial x_2}, \dots] = (\nabla f(x))^T$
"""

# ╔═╡ af4d4227-8d44-43e8-96cf-e379fe9a72ca
md"""
### Part 2
"""

# ╔═╡ f120f341-5215-4a95-9922-9a9e8b352a72
md"""
$NN(u, W_1, W_2, b_1, b_2) = W_2\tanh.(W_1u+b_1)+b_2$
"""

# ╔═╡ 221cecbd-fb74-4606-ad36-33553861e630
NN(u::SVector{2}, W₁::SMatrix{50, 2}, W₂::SMatrix{2, 50}, b₁::SVector{50}, b₂::SVector{2}) = W₂ * tanh.(W₁ * u + b₁) + b₂;

# ╔═╡ 0a698374-235a-4acb-a3da-78f76c6c6f82
function B_NN(ȳ::SVector{2}, u::SVector{2}, W₁::SMatrix{50, 2}, W₂::SMatrix{2, 50}, b₁::SVector{50}, b₂::SVector{2})
	z = W₁ * u + b₁
	h = tanh.(z)
	h̄ = W₂' * ȳ
	z̄ = h̄ .* (1 .- h .^ 2)
	(; ū=W₁' * z̄, W̄₁=z̄ * u', W̄₂=ȳ * h', b̄₁=z̄, b̄₂=ȳ)
end;

# ╔═╡ 434d9e75-0b1f-4137-a576-bd2dfabadad1
begin
	u = @SVector rand(2)
	W₁ = @SMatrix rand(50, 2)
	W₂ = @SMatrix rand(2, 50)
...
# ╔═╡ 09422932-deb5-412e-b4d3-1fd9c26750d5
begin
	B₁ = B_NN(@SVector[1,0], u, W₁, W₂, b₁, b₂)
	B₂ = B_NN(@SVector[0,1], u, W₁, W₂, b₁, b₂)
end;

# ╔═╡ b3c451a5-51e6-489e-9201-789eee9f115f
begin
	J_u = ForwardDiff.jacobian(u -> NN(u, W₁, W₂, b₁, b₂), u)
	J_W₁ = ForwardDiff.jacobian(W₁ -> NN(u, W₁, W₂, b₁, b₂), W₁)
	J_W₂ = ForwardDiff.jacobian(W₂ -> NN(u, W₁, W₂, b₁, b₂), W₂)
	J_b₁ = ForwardDiff.jacobian(b₁ -> NN(u, W₁, W₂, b₁, b₂), b₁)
	J_b₂ = ForwardDiff.jacobian(b₂ -> NN(u, W₁, W₂, b₁, b₂), b₂)
end;

# ╔═╡ 387d8e30-2d0d-49d4-802f-e5d1eb2cdb10
@testset begin
	@test hcat(B₁.ū, B₂.ū)' ≈ J_u
	@test hcat(vec(B₁.W̄₁), vec(B₂.W̄₁))' ≈ J_W₁
	@test hcat(vec(B₁.W̄₂), vec(B₂.W̄₂))' ≈ J_W₂
	@test hcat(B₁.b̄₁, B₂.b̄₁)' ≈ J_b₁
	@test hcat(B₁.b̄₂, B₂.b̄₂)' ≈ J_b₂
end;

# ╔═╡ aed0ede5-edfb-4811-ae7a-fb757791f5ba
md"""
### Part 3
"""

# ╔═╡ dd4b3105-bc8b-4faa-b40e-594aa3f2d73c
flatten(W₁::SMatrix{50, 2}, W₂::SMatrix{2, 50}, b₁::SVector{50}, b₂::SVector{2}) = SVector{252}([vec(W₁)..., vec(W₂)..., b₁..., b₂...]);

# ╔═╡ 8d325c4e-a4f5-42d3-86dc-dbff17a18552
unflatten(p) = SMatrix{50,2}(reshape((@view p[1:100]), 50, 2)), SMatrix{2,50}(reshape((@view p[101:200]), 2, 50)), SVector{50}(@view p[201:250]), SVector{2}(@view p[251:252]);

# ╔═╡ 526b719c-e770-4178-9fff-fea202ed7908
function forward_pass(u₀, p, ts)
	f(u, p, t) = NN(u, unflatten(p)...)
	prob = ODEProblem(f, u₀, (ts[1], ts[end]), p)
	solve(prob, Tsit5())
end;

# ╔═╡ c0ae925a-6cc7-4721-ba14-5be3223961b2
function backward_pass(u_sol, p, ts, Cᵤ)
	function f(u, p, t)
		B = B_NN(SVector{2}(u[1:2]), u_sol(t), unflatten(p)...)
		-vcat(B.ū, flatten(B.W̄₁, B.W̄₂, B.b̄₁, B.b̄₂))
	end
	prob = ODEProblem(f, vcat(Cᵤ(u_sol, ts[end]), zeros(length(p))), (ts[end], ts[1]), p)
	
	jumps = ts[1:end-1]
	affect!(integrator) = integrator.u[1:2] .+= Cᵤ(u_sol, integrator.t)
	cb = PresetTimeCallback(jumps, affect!)
	
	solve(prob, Tsit5(), callback=cb)(0)[3:254]
end;

# ╔═╡ bf1d2f90-b5ee-4294-89ee-200e15eed114
function generate_data(u₀, ts)
	A = @SMatrix [-0.1 2.0; -2.0 -0.1]
	prob = ODEProblem((u,p,t) -> A * u, u₀, (ts[1], ts[end]))
	solve(prob, Tsit5(), saveat=ts)
end;

# ╔═╡ c9d87095-fb9b-41bd-8939-bea2d1d1fcac
begin
	u₀ = @SVector [2,0]
	p₀ = [W₁, W₂, b₁, b₂]
	ts = 0.0:0.1:1.0
	û = generate_data(u₀, ts)
	Cᵤ(u_sol, t) = 2(u_sol(t) - û(t))
end;

# ╔═╡ 347c7a51-9e13-495e-a2e3-40df73a3e8de
begin
	u_sol = forward_pass(u₀, flatten(p₀...), ts)
	gradient = backward_pass(u_sol, flatten(p₀...), ts, Cᵤ)
	unflatten(gradient)
end

# ╔═╡ 1c71bacf-763a-404c-99be-2c743eb1d4bc
md"""
### Part 4
"""

# ╔═╡ 2be04d43-a5c2-4800-a5da-ab4440eafda8
function gradient_descent(u₀, p₀, ts, C, Cᵤ, η, T)
	p = flatten(p₀...)
	loss = Vector{Float64}(undef, T)
	for i ∈ 1:T
		u_sol = forward_pass(u₀, p, ts)
		loss[i] = C(u_sol)
		p -= η * backward_pass(u_sol, p, ts, Cᵤ)
	end
	unflatten(p), loss
end;

# ╔═╡ 9d79e767-a371-4ef4-938e-1171d18856fa
begin
	C(u_sol) = mean(sum((u_sol(t) - û(t)) .^ 2) for t ∈ ts)
	η = 0.001
	T = 1000
	p, loss = gradient_descent(u₀, p₀, ts, C, Cᵤ, η, T)
	loss[end]
end

# ╔═╡ 59d24d17-6b0d-471f-aaaa-013cdce25c4e
plot(1:T, loss, title="L2 Loss", yaxis=:log, legend=false)

# ╔═╡ 2b1fc8b8-9de8-442a-b25d-7e73fc46fa6e
begin
	converged_sol = forward_pass(u₀, flatten(p...), ts)
	plot(converged_sol; title="Converged Solution", lw=2)
	scatter!(û, vars=(0,1), color=1, label="û₁(t)")
	scatter!(û, vars=(0,2), color=2, label="û₂(t)")
end
